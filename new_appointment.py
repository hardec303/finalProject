...
class Createnewappointment(Toplevel):
    def __init__(self, name):
        ...
        query = "select * from client_info where full_name = '{}'".format(self.client_name)
        result = cur.execute(query).fetchone()

        self.id = result[0]
        business_phone = result[4]
        phone = result[3]
        email = result[9]
        #  query2="select * from appointment where full_name = '{}'".format(self.clientID)
        ...
        self.heading = Label(self.top, text=self.client_name,
                             font='arial 18 ')
        ...
        self.entry_appointment_name.insert(0, "")
        ...
        self.entry_appointment_info.insert(0, "")
        ...
        self.lablel_phonenumber = Label(self.bottom, text=business_phone, font='arial 15')
        ...
        self.btnback = Button(self.bottom, text="Add Appointment", font='arial 12 ', width=15, height=2,
    ...
    def add_appointment(self):
        ...
        #checks if there are blank fields before further error checking
        if appointment_name == "" or appointment_date == "" or appointment_information == "":
            ...
            if appointment_date.count('-') != 2 or appointment_date.count(':') != 1 or appointment_date.count(' ') != 1:
                messagebox.showinfo("Error",
                                    "Please enter the date in the YYYY-MM-DD H:M format, you are missing some special characters.")
                ...
                if len(date_list[0]) != 4 or len(date_list[1]) != 2 or len(
                    ...
                    messagebox.showinfo("Error", "Please enter the date in the YYYY-MM-DD H:M format, the length of some numbers is incorrect.")
                    appointment_error = True
                    self.destroy()
                ...
                if date_list[0].isnumeric() is False or date_list[1].isnumeric() is False or day_time_list[0].isnumeric() is False or hour_minute_list[0].isnumeric() is False or hour_minute_list[1].isnumeric() is False:
                ...
                if int(date_list[0]) == int(current_year) and int(date_list[1]) == int(current_month) and int(day_time_list[0]) < int(current_day):
                    messagebox.showinfo("Error","you have entered a date that is prior to the current date.")
                    appointment_error = True
                    self.destroy()
                ...
                if appointment_error is False:
                    try:
                        query = "insert into 'appointment'('client_id','full_name','appointment_name','appointment_date','appointment_information') values(?,?,?,?,?)"
                        cur.execute(query,
                                    (client_id, full_name, appointment_name, appointment_date, appointment_information))
                        con.commit()
                        messagebox.showinfo("Success", "Appointment was added successfully.")
                        self.destroy()
                    except:
